...
void ps_send_to_pagespeed(ngx_http_request_t* r,
  ...
  for (cur = in; cur != NULL; cur = cur->next) {
    ...
    if (cur->buf->flush && ctx->follow_flushes) {
      ...
      // unless we have flush_html enabled. Note that PSOL might aggregate
      // multiple flushes into 1, and actually flush a little bit later due to
      // html parser state and earlier scheduled operations.
      ...
      // TODO: Add an option to PSOL to forward upstream flushes, separate from
      // flush_html.
